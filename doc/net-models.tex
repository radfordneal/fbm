...
approximation above takes the maximum with respect to $u$.  For the
$\tanh$ activation function, $\max[a'(u)] = 1$, so the end result 
...
\beq
  {\partial^2 L \over \partial (v^S_i)^2} & \approx &
     \sum_D\, \sum_{(j,k)\,:\,(i,j,k)\in C^{S,D}}\!\!\!\!
       (\sigma^{S,D}_w)^2\, {\partial^2 L \over (v^D_j)^2}
\eep
...
which is the maximum possible value for $\tanh$ units.  With weights
for configured connections, an analogous formula is used that sums over
the connections for which the weight is used.

Second derivatives with respect to biases and offsets are simply equal
to the second derivatives with respect to the associated summed input to
the hidden or output units.
