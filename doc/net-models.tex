...
%
  ...
  $b^O_j$            & Bias for $j$th output unit \\[12pt]
  ...
  $C^{I,\ell}$       & Set of triples, $(i,j,k)$, giving configuration of
                       connections from the input layer to hidden 
                       layer $\ell$ \\[3pt]
  $C^{\ell\!-\!1,\ell}$  & Set of triples, $(i,j,k)$, giving configuration of
                       connections from hidden layer $\ell\!-\!1$ to hidden 
                       layer $\ell$\\[18pt]
%
  \multicolumn{2}{c}{\bf Hyperparameters defining priors for parameters} \\[8pt]
%
  ...
  $\sigma^{\ell}_b$   & Common sigma for biases of units in hidden layer 
                      $\ell$ \\[3pt]
  $\sigma^O_b$        & Common sigma for biases of output units \\[12pt]
  ...
  $\sigma^{\ell,O}_w$    & Common sigma for weights from units in hidden
                           layer $\ell$ to output units \\[12pt]
...
\label{func3}\eeq% 
Here, $a^{\ell}$ is the activation function used for layer $\ell$ (eg, $\tanh$).
The summations are over all units in the
appropriate layer.  (The number of units in each layer is part of the
architecture specification, but these numbers are not given symbols
here.)  The term in the equation for $u^{\ell}_i$ involving layer
$\ell\!-\!1$ is omitted for the first hidden layer; terms involving 
other pairs of layers will also be omitted when those connections are
absent from the architecture.

Connections from an input layer to a hidden layer may not be fully
connected, but instead be configured by specifying a set, $C^{I,\ell}$
of triples, $(i,j,k)$, indicating that unit $i$ in the input layer
contributes to the input of unit $j$ of the hidden layer, using the
weight indexed by $k$.  The set of weights referenced may be smaller
(or larger) than the number in that would be present when each input
unit connects to each hidden unit, with an unshared weight.  The
...
than one weight.  Similarly, connections from a hidden layer to the
next hidden layer may be specified by a set of triples
$C^{\ell\!-\!1,\ell}$.

When the connections from inputs and the previous hidden layer are
configured in this way, the input to a unit is computed as follows:\beq
  u^{\ell}_j & = & b^{\ell}_j \ +\!\!\!
...
\label{func1-c}\eeq%
When one set of connections into a hidden layer is configured in this
way, and the other is fully-connected, the appropriate combination of
equations \ref{func1} and \ref{func1-c} is used.
...
explicitly represented noise sigmas fixed) are obtained using the
derivatives with respect to unit values and unit inputs found in the
previous section, as follows:\beq
...
second derivative of $L$ with respect to the summed input to a $\tanh$
hidden unit, given its second derivative with respect to the unit's
output.  Letting the hidden unit output be $v = \tanh(u)$, we have\beq
  {d^2L \over du^2}
   \ = \ {d \over du} \left[ (1-v^2) {dL \over dv} \right] 
   \ = \ (1-v^2) {d^2L \over dv^2} - 2v {dL \over dv} \label{sechterm}
   \ \approx \ (1-v^2) {d^2L \over dv^2} 
   \ \approx \ {d^2L \over dv^2}\ \ \
\eeq%
The first approximation assumes that since $2v(dL/dv)$ may be either
positive or negative, its effects will (optimistically) cancel when
averaged over the training set.  Since $v$ is not known, the second
approximation above takes the maximum with respect to $v$.  The end result 
is that we just ignore the fact that the hidden unit input is passed 
through the $\tanh$ function.
...
can be expressed as follows:\beq
  {\partial^2 L \over \partial (w^{S,D}_{i,j})^2} & = &
    (v^S_i)^2\,{\partial^2 L \over \partial (v^D_j)^2}
\eeq%
When the weight is on a connection from an input unit, $v^S_i = v^I_i$ is 
the $i$th input for this training case, which is known.  If the weight
is on a connection from a hidden unit, $(v^S_i)^2$ is assumed to be one,
the maximum possible value.  

Second derivatives with respect to biases and offsets are simply equal
to the second derivatives with respect to the associated unit values.
...
summed for all cases in the training set.  To these are added
estimates of the second derivatives of the log prior probability with
respect to each parameter, giving estimates of the second derivatives
...
Finally, the stepsize used for a parameter is the reciprocal of the
square root of minus the estimated second derivative of the log
posterior with respect to that parameter.
