...
/* EVALUATE NETWORK FUNCTION OF INPUTS FOR 2 CASES, USING 2 GPU THREADS.
   ...
   The computation uses two threads, identified by 'th' being 0 or 1,
   but may be called with 'th' greater than 1, for which it does
   nothing except sync at the same points as other threads will sync.

   Layers are computed in sequence, using both threads, with a
   __syncthreads call after each layer's computations, so that both
   threads will correctly see all values when computing the next layer.

   Thread 0 is used to compute the even units for both cases, thread 1
   is used to compute the odd units for both cases, except that when
   there is only one unit in a layer, thread 0 is used for case 0 and
   thread 1 for case 1.  Consistent use of this scheme for the various
   componenets avoids any need to synchronize threads within computations
   for a single layer. 
*/

__device__ void net_func2
( int th,		/* Which thread, 0 or 1, or greater to just sync */
...
{ if (omit==0)
  { if (off==0)
    ...
    { ADD_CONNECTIONS2(*off++,0);
    }
  }
  else
  { if (off==0)
    { ADD_CONNECTIONS2(0,(*omit++)&ob);
    }
    else
    { ADD_CONNECTIONS2(*off++,(*omit++)&ob);
    }
  }
}
