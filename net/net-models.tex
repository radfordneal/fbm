...
non-sequential connections are handled in the obvious way, with this
extension not otherwise noted in this presentation.)
...
For the $\tanh$ and softplus activation functions, $\max[a'(u)^2] = 1$.
For $a(u)=\sqrt{2}\sin(\sqrt{2}u)$, we have $\max[a'(u)^2] = 4$.  For
the square and cube activation functions, there is no bound on $a'(u)^2$;
so rather than take the maximum, the value for $a'(u)^2$ at a ``typical''
value for $u$ is used, with typical values found as described below.
...
this way is reduced to one if it exceeds one, since one is the maximum
squared value of tanh.  Similarly, the typical squared value for a
unit with activation function $\sqrt{2}\,\sin(\sqrt{2}u)$ is capped at
two.  Units with the softplus activation function are treated the same
as those with identity activation function.  For units with the square
or cube activation function, the typical squared value is the square
or cube of the typical squared value of the input to the unit.
