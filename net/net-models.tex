...
hidden layer.  In the second equation, $a'^{\ell}(u^{\ell}_i)$ is the
derivative of the activation function, evaluated at $u^{\ell}_i$.  For
computational reasons, it is convenient to express this derivative in
terms of $v^{\ell}_i$.  For the $\tanh$ activation function, the
derivative of $L$ is $1 - \big[v^{\ell}_i\big]^2\!$.  For the softplus
activation function, it is $1 - \exp(-v^{\ell}_i)$, and for
softplus0, it is $1 - \exp(-v^{\ell}_i-\log(2)^2))$.
